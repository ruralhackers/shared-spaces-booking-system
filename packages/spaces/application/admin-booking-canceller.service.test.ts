...
import { AdminBookingCanceller } from './admin-booking-canceller.service'
...
describe('The AdminBookingCanceller service', () => {
  test('cancels without name verification', async () => {
    ...
    const service = new AdminBookingCanceller(spaceRepo, bookingRepo, notifier, clock)

    await service.run({ id: booking.toDto().id })

    const updated = await bookingRepo.findById(booking.toDto().id)
    expect(updated?.toDto().status).toBe('cancelled')
    expect(updated?.toDto().cancelledBy).toBe('admin')
  })

  test('notifies with "admin" attribution', async () => {
    const spaceRepo = new InMemorySpaceRepository()
    const bookingRepo = new InMemoryBookingRepository()
    const notifier = new InMemoryNotifier()
    spaceRepo.seed([chillHouse])
    ...
    expect(notifier.calls[0]?.by).toBe('admin')
  })
  ...
  test('rejects missing booking', async () => {
    ...
    expect(service.run({ id: 'nonexistent' })).rejects.toThrow(BookingNotFoundError)
  })
  ...
  test('rejects already-cancelled booking', async () => {
    ...
    expect(service.run({ id: booking.toDto().id })).rejects.toThrow(BusinessRuleError)
  })
})
